...
class Command(BaseCommand):
    def handle(self, *args, **options):
        self.stdout.write("Start making bank data\n")

        data = load_json("data", "bank_data.json")
        ...
        for bank in bank_data_list:
            if Facility.objects.filter(name=bank["장소명"], type=facility_type).exists():
                continue
            url = (
        ...
        self.stdout.write("\n")
        self.stdout.write("Make Bank Data is Done. :D\n")
        self.stdout.write(
            "------------------------------------------------------------\n"
        )
